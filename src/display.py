...
def draw_cell(screen, edges: dict, coord: tuple, edge_length: float, EPSILON : float) -> None:
    if edges['N']:
...
def draw_player(screen, maze: Maze):
...
def draw_maze(screen, maze: Maze) -> None:
    edge_length = maze.edge_length
    for cell in maze.grid.keys():
        cell = cell[1], cell[0]
        draw_cell(screen, maze.grid[cell], (MAZE_LEFT_CORNER[0] + edge_length * cell[1],
                  MAZE_LEFT_CORNER[1] + edge_length * cell[0]), edge_length, maze.EPSILON)
    draw_player(screen, maze)


def draw_cell_in_path(screen, maze: Maze, cell_index: int) -> None:
...
def draw_path(screen, maze: Maze) -> None:
    for i in range(maze.current_cell_in_path):
        draw_cell_in_path(screen, maze, i)
